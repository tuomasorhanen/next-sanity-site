                      ...
                      {item.subPages.map(subItem => (
                        <li key={subItem._id}>
                          <Link href={'/' + subItem.slug.current}>
                            <div className="block px-4 py-2 text-text hover:opacity-50">{subItem.name}</div>
                          </Link>
                        </li>
                      ))}
                    </ul>
      ...
      <nav className="nav z-40 md:hidden">
        <div className="nav-container">
            ...
            <div className="flex items-center">
              <div className="menu-toggle" onClick={() => setNavOpen(!navOpen)}>
                <div className={navOpen ? 'hamBox hamBoxOpen' : 'hamBox'}>
                  <span className={navOpen ? `lineTop spin bg-bg` : `lineTop bg-black`}></span>
                  <span className={navOpen ? `lineBottom spin bg-bg` : `lineBottom bg-black`}></span>
                </div>
              </div>
            </div>
          </div>
          <div
            ...
            style={{ top: navOpen ? '0' : '-100%', transitionDelay: navOpen ? '0s' : '0s' }}>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {item.hasSubPages && (
                      <button
                        ...
                        <svg
                          className={`ml-2 h-6 w-6 transform ${openSubPage === item.slug.current ? 'rotate-90' : ''}`}
                          fill="none"
                          stroke="white"
                          viewBox="0 0 24 24"
                          xmlns="http://www.w3.org/2000/svg">
                  ...
                  {item.hasSubPages && item.subPages && (
                    <ul
                      className={`submenu-transition ${
                        openSubPage === item.slug.current ? 'submenu-open mb-2' : 'max-h-0'
                      }`}>
                      {item.subPages.map(subItem => (
                        <li key={subItem._id} className="nav-sub-item py-1 pl-4 text-left text-sm">
                          <Link href={'/' + subItem.slug.current} onClick={() => setNavOpen(!navOpen)}>
                            {subItem.name}
                          </Link>
                          <div className="nav-item-wrapper"></div>
                        </li>
